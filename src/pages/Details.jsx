...
export default function Details() {
  ...
  useEffect(() => {
    idRef.current = id;
    fetch(`${API_BASE}/${category}/${idRef.current}`)
      .then(res => res.json())
      .then(json => {
  ...
  if (!properties) return <div className="text-center my-4">Loading…</div>;

  const isFav = favorites.some(
    f => f.category === category && f.id === idRef.current
  );

  const imgSrc = `https://picsum.photos/seed/${category}-${idRef.current}/288/180`;
